...
const Appointment = () => {
  ...
  return docInfo && (
    <div >
      ...
      <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-800] sm:mt-0'>
        {/*name degree and experience */}
        <p  className='flex items-center gap-2 text-2xl font-medium text-gray-900 '>{docInfo.name} <img className='w-5' src={assets.verified_icon} alt="" /></p>

        <div className=' flex items-ceter gap-2 text-gray-600 mt-1'>
        <p>{docInfo.degree} - {docInfo.speciality}</p>
        <button className='ox-0.5 px-2 border text-xs rounded-full'> {docInfo.experience}</button>
        </div>
      ...
      <div className='flex gap-3 items-center overflow-x-scroll mt-4 w-full'>
        {
          ...
          )) :<div><p>No slots Available</p></div>
        }
